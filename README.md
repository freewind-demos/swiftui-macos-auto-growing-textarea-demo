...
核心做法不是自己桥接 `NSTextView`，而是直接使用 SwiftUI 原生 `TextField(axis: .vertical)`，再用本地 `keyDown` 监听，把 plain `Return` 直接转给当前 field editor 的 `insertLineBreak:`。
...
3. 好处：这次不是全局 `sendAction`，而是只命中当前编辑中的文本对象，并尝试走更贴近尾部换行的 `insertLineBreak:` 路径。
